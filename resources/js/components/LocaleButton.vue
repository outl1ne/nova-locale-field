<template>
  <button type="button" :class="classes" class="locale-button" v-if="localeDisplayName" ref="localeButton">
    ...
    <!-- Hidden select -->
    <select v-if="!disabled" name="locale-options" :value="selectedLocale" @change="localeChanged" class="fake-select">
      <option
...
export default {
  ...
  computed: {
    ...
    selectedLocale() {
      return this.locale || (this.field.value && this.field.value.locale) || null;
    },

    localeDisplayName() {
      const matchedLocale = this.field.locales.find(l => l.value === this.selectedLocale);
